...
export const Header: React.FC<HeaderProps> = ({ 
  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-16 bg-background/80 backdrop-blur-md border-b border-border flex items-center justify-between px-4 z-50 transition-colors duration-500">
      ...
      >
        {/* Custom SVG Logo */}
        <div className="h-8 sm:h-10 w-auto aspect-[1.8/1] relative transform transition-transform group-active:scale-95">
        ...
        <span className="text-base sm:text-xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-foreground to-slate-400">
            StrangerConnect
        </span>
      </button>
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <button 
            ...
            className="p-2 rounded-full text-slate-400 hover:text-primary hover:bg-surface border border-transparent hover:border-border transition-all active:scale-95"
            aria-label="Settings"
        >
        ...
        {appState === AppState.LANDING && (
           <div className="flex items-center gap-2 text-xs sm:text-sm text-slate-400">
             ...
             {onlineCount.toLocaleString()} <span>online</span>
           </div>
        )}
        ...
        {appState === AppState.CHAT && (
          <button 
            onClick={onLeaveChat}
            className="text-xs sm:text-sm font-medium text-red-400 hover:text-red-300 transition-colors px-3 py-1.5 rounded-full hover:bg-red-500/10 border border-red-500/20 active:scale-95"
          >
            Disconnect
          </button>
        )}
